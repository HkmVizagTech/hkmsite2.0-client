/**
 * Ekadashi campaign page configuration.
 *
 * The /ekadashi page is driven by a single DB document (server singleton
 * keyed "ekadashi"). When the admin has not edited it yet, the controller
 * merges with DEFAULT_CAMPAIGN. This client-side copy exists as a last-resort
 * fallback if the API is unreachable at SSR time.
 */
...
export interface EkadashiUnit {
  price: number;
  singular: string;
  plural: string;
}

export interface EkadashiSeva {
  key: string;
  label: string;
  icon: string;
  sevaName: string;
...
export interface EkadashiSevaCard {
  title: string;
  description: string;
  image: string;
  href: string;
  /** Icon key: utensils | heart | star | book | flower (default). */
  icon?: string;
}
...
export interface EkadashiFaq {
  q: string;
  a: string;
}

export interface EkadashiShloka {
  sanskrit: string;
  translation: string;
  reference: string;
}

export interface EkadashiBankDetails {
...
export interface EkadashiCampaign {
  campaignName: string;
  pageTitle: string;
  metaTitle: string;
  metaDesc: string;
  ...
  orderType: string;
  sevas: EkadashiSeva[];
  sevaCards: EkadashiSevaCard[];
  significancePoints: TextBlock[];
  whyDonateSections: TextBlock[];
  faqs: EkadashiFaq[];
  shloka: EkadashiShloka;
  bankDetails: EkadashiBankDetails;
}

/* ------------------------------------------------------------------ */
/* Default campaign (matches the server defaults)                      */
/* ------------------------------------------------------------------ */

export const DEFAULT_CAMPAIGN: EkadashiCampaign = {
  campaignName: "Shayani Ekadashi",
  pageTitle: "Shayani Ekadashi Seva",
  metaTitle:
    "Shayani Ekadashi Seva | Hare Krishna Vaikuntham Temple, Visakhapatnam",
  metaDesc:
  ...
  shloka: {
    sanskrit:
      "एकादश्या यतः पुण्यं ततः कोटिगुणं भवेत् । अश्वमेधशतं चैव विष्णोर्नामस्मरणं तथा ॥",
    translation:
...
export const ekadashiApiBase = () =>
  (process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080").replace(
    /\/+$/,
    ""
  );

/** Fetch the merged campaign config from the server (used in SSR). */
export async function fetchEkadashiCampaign(): Promise<EkadashiCampaign> {
  try {
    const res = await fetch(`${ekadashiApiBase()}/ekadashi-campaign`);
    if (!res.ok) return DEFAULT_CAMPAIGN;
    const data = (await res.json()) as Partial<EkadashiCampaign>;
    return { ...DEFAULT_CAMPAIGN, ...data } as EkadashiCampaign;
  } catch {

...
// PageLayout + the homepage) so it appears on every page, not just donation
// campaign pages. Deliberately NOT added to /donations — per standing
// instruction, that page is only ever touched when explicitly requested by name.

// Module-level guard: several campaign pages still render their own copy of
// this button, so only the first instance to mount ever displays an icon.
let mounted = false;
...
export default function WhatsAppFloatButton({
  ...
  return (
    <a
      ...
      // bottom-20 keeps the button clear of the fixed mobile bottom nav
      // (raised above it); md: resets to a small margin from the viewport.
      className="fixed bottom-20 right-5 z-50 flex h-11 w-11 items-center justify-center rounded-full bg-[#25D366] shadow-lg transition-transform hover:scale-110 active:scale-95 md:bottom-6 md:right-6"
    >
      <svg viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 fill-white">

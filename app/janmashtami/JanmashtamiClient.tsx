...
export default function JanmashtamiClient({ campaigner }: { campaigner?: JanmashtamiCampaigner } = {}) {
  ...
  const canSubmit = !!selected?.option && !!finalAmount && finalAmount >= 100;
  ...
  const selectedSummary = useMemo(() => {
    if (!selected) return "";
    return `${selected.seva.title} - ${selected.option?.label ?? "Select an amount"}`;
  }, [selected]);
  ...
  const selectOption = (option: SevaOption) => {
    setSelected((current) => (current ? { seva: current.seva, option } : current));
    updateForm({ customAmount: "" });
    setStatus({ type: "idle", message: "" });
  };
  ...
  const validate = () => {
    ...
    if (!selected.option) return "Please select an amount.";
  ...
  const submitDonation = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!selected || !selected.option) return;
    setSubmitting(true);
    setStatus({ type: "idle", message: "" });
          ...
          >
            <div className="overflow-hidden rounded-2xl border border-amber-300/70 bg-white shadow-[0_14px_35px_rgba(120,60,10,0.14)]">
              ...
              <form onSubmit={submitDonation} className="space-y-5 p-6">
                ...
                {/* Amount chips for the selected seva */}
                {selected && (
                  <div>
                    <span className="mb-2 block text-sm font-semibold text-[#331447]">Offer Amount *</span>
                    <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                      {selected.seva.options.map((option, optIdx) => {
                        const isActive = selected.option?.legacySevaId === option.legacySevaId;
                        ...
                        return (
                          <button
                            ...
                            className={`
                              relative flex flex-col items-center justify-center gap-0.5 rounded-xl border px-3 py-3 text-center transition-all duration-300
                              ${isCustom ? "col-span-2 sm:col-span-1" : ""}
                              ${isActive
                                ? "border-amber-600 bg-gradient-to-br from-amber-400 to-orange-300 text-[#3b1605] shadow-[0_4px_14px_rgba(217,119,6,0.35)]"
                                : isTopTier
                                  ? "border-amber-500/80 bg-gradient-to-b from-amber-100 to-orange-100 text-[#5c2e06] hover:border-amber-500"
                          ...
                          >
                            {option.amount ? (
                              <span className="text-[13px] font-bold md:text-sm">
                                <span className="text-[11px] font-normal text-amber-800/80">₹</span> {formatAmount(option.amount)}
                              </span>
                            ) : (
                              <span className="flex items-center justify-center gap-1.5 text-[12px] font-bold">
                                <svg className="h-3 w-3 text-amber-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
                                Other Amount
                              </span>
                            )}
                            {hasSubtitle && (
                              <span className="inline-block rounded-full bg-gradient-to-r from-[#5c1a0b] to-[#7a2e0f] px-2.5 py-0.5 text-[9px] font-bold uppercase tracking-wider text-amber-100">
                                {option.subtitle}
                              </span>
                            )}
                          </button>
                ...
                {selected?.option && !selected?.option?.amount && (
